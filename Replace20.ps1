$filesToChange = Get-ChildItem -Exclude globalParameters, PartialArmTemplates | foreach-object { Get-ChildItem $_ -File -Recurse } | Where-Object { $_.Name -notlike "*.ps1" -and $_.Name -like "*.json" }
...
foreach ($file in $filesToChange) {
    ...
    foreach ($name in $allParametrsList.Name) {
        ...
        $replacedDefaultValue = $defaultValue -replace 'dev', 'prod'
        ( Get-Content $file -Raw ) -replace "$defaultValue", "$replacedDefaultValue" | Set-Content $file
